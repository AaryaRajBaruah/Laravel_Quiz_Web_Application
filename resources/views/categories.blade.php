...
<body>
     <x-navbar name={{$name}}></x-navbar>                                    <!-- imported the navbar  -->
    @if(session('category'))
    <div class=" bg-green-800 text-white pl-3">{{session('category')}}</div>
    @endif
<div  class=" bg-gray-100 flex  flex-col  items-center min-h-screen pt-5">
    ...
    <form action="/add-category " method="post" class="space-y-4">
        @csrf
        <div>
            <input type="text" placeholder="Enter category here" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none " name="category">
           @error('category')
